...
- [ ] Event approval rejects creators whose account verification is not currently active; the admin records only account verification metadata/history after confirming the creator is 18+, the emailed government-ID image is deleted, no ID image/DOB/number appears in application storage or logs, and each event still receives separate review plus an approval-time verification snapshot.
