...
class PromoCode(TimestampMixin, Base):
    ...
    __table_args__ = (UniqueConstraint("event_id", "code_normalized", name="uq_promo_codes_event_id_code_normalized"),)
    ...
    discount_type: Mapped[PromoCodeDiscountType] = mapped_column(
        Enum(PromoCodeDiscountType, name="promo_code_discount_type"), nullable=False
    )
    ...
    applies_to_all_tiers: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True)
    ...
    tier_scopes: Mapped[list["PromoCodeTicketTier"]] = relationship(back_populates="promo_code", cascade="all, delete-orphan")
