...
class EventRefundBatch(TimestampMixin, Base):
    ...
    initiated_by_user_id: Mapped[int | None] = mapped_column(ForeignKey("users.id"), nullable=True, index=True)
    status: Mapped[EventRefundBatchStatus] = mapped_column(
        Enum(EventRefundBatchStatus, name="event_refund_batch_status"),
        nullable=False,
    ...
    initiated_by_user: Mapped["User | None"] = relationship(foreign_keys=[initiated_by_user_id])
