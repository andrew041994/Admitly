...
class MessageDeliveryLog(TimestampMixin, Base):
    ...
    template_type: Mapped[MessageTemplateType] = mapped_column(
        Enum(MessageTemplateType, name="message_template_type"), nullable=False, index=True
    )
    channel: Mapped[MessageChannel] = mapped_column(
        Enum(MessageChannel, name="message_channel"), nullable=False, index=True
    )
    status: Mapped[MessageDeliveryStatus] = mapped_column(
        Enum(MessageDeliveryStatus, name="message_delivery_status"), nullable=False, index=True
    )
    ...
    is_manual_resend: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    resend_of_message_id: Mapped[int | None] = mapped_column(ForeignKey("message_delivery_logs.id"), nullable=True, index=True)
    ...
    resend_of: Mapped["MessageDeliveryLog | None"] = relationship(remote_side=[id], uselist=False)
