...
class EventStaff(TimestampMixin, Base):
    ...
    __table_args__ = (
        UniqueConstraint("event_id", "user_id", name="uq_event_staff_event_id_user_id"),
    )
    ...
    role: Mapped[EventStaffRole] = mapped_column(
        db_enum(EventStaffRole, name="event_staff_role"),
        nullable=False,
    ...
    user = relationship(
        "User", foreign_keys=[user_id], back_populates="event_staff_assignments"
    )
    invited_by_user = relationship(
        "User",
        foreign_keys=[invited_by_user_id],
        back_populates="invited_staff_records",
    )
