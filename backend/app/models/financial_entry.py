...
class FinancialEntry(TimestampMixin, Base):
    ...
    organizer_id: Mapped[int | None] = mapped_column(ForeignKey("organizer_profiles.id"), nullable=True, index=True)
    amount: Mapped[Decimal] = mapped_column(Numeric(10, 2), nullable=False)
    entry_type: Mapped[FinancialEntryType] = mapped_column(
        Enum(FinancialEntryType, name="financial_entry_type"), nullable=False, index=True
    )
