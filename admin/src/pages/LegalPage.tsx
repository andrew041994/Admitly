...
const documents: Record<string, LegalDocument> = {
  privacy: {
    ...
    sections: [
      ...
      { heading: 'Event-creator age and identity verification', body: 'Before an event can be approved, its creator may upload a valid government-issued ID through Admitly solely so an authorized administrator can verify identity and that the creator is at least 18. The image is held temporarily in restricted private storage, is not public, and is not stored in the application database. Admitly attempts to delete it immediately after verification or rejection. If immediate deletion fails, the record is marked for controlled cleanup and deletion is retried; a seven-day storage lifecycle is a backstop for abandoned material, not ordinary retention. Verification applies to the creator account, so a verified creator generally does not need to resubmit identification for each future event. Admitly may require reverification where reasonably necessary for security, fraud prevention, legal compliance, verification error, or identity concerns. Admitly retains verification status and history, verifier administrator ID, verification time, optional non-document notes, deletion/cleanup evidence, and an event approval-time verification snapshot—not the ID image. Admitly does not intentionally retain date of birth or government ID number as verification metadata.' },
  ...
  terms: {
    ...
    sections: [
      { heading: 'Using Admitly', body: 'You must provide accurate information, keep account credentials secure, and use the service only for lawful ticketing activity. Any authenticated user may create an event, but an event creator must be at least 18 and have an actively verified creator account before an event may be approved or published. Verification generally carries forward to later events unless Admitly reasonably requires reverification.' },
      { heading: 'Event review', body: 'Event creators remain responsible for event accuracy, legality, safety, required permits and rights, and delivery. Admitly may withhold approval or publication until age and identity verification and event review are complete. A government-issued ID may be uploaded through the authenticated creator workflow for temporary, restricted review by authorized Admitly administrators and is subject to the deletion and cleanup process described in the Privacy Policy.' },
  ...
  organizers: {
    ...
    sections: [
      { heading: 'Age and identity verification', body: 'An event creator must be at least 18 and have their creator account verified before an event may be approved or published. A creator may upload a government-issued ID through the authenticated Admitly workflow for manual review by authorized administrators. The image is temporarily held in restricted private storage, is not stored in the application database, and is scheduled for immediate deletion after verification or rejection. Failed deletion is flagged for controlled retry, with a seven-day storage lifecycle backstop for abandoned material. Once verified, a creator generally does not need to resubmit identification for each future event, although Admitly may require reverification for legitimate security, identity, fraud-prevention, or compliance reasons. Admitly retains safe verification status, history, and cleanup evidence—not the ID image, date of birth, or government ID number.' },
      ...
      { heading: 'Review and enforcement', body: 'Admitly may withhold event approval or publication until account age/identity verification and event review are complete, may revoke verification and require reverification where reasonably justified, and may suspend activity where reasonably necessary for fraud, security, safety, legal, refund, or data-integrity review. Revocation does not itself cancel or unpublish an already-approved event. This does not create fee, reserve, or payout terms beyond those expressly stated.' },
    ],
  },
  buyers: {
    title: 'Buyer Terms',
