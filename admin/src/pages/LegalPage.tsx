...
const documents: Record<string, LegalDocument> = {
  privacy: {
    ...
    sections: [
      ...
      { heading: 'Event-creator age and identity verification', body: 'Before an event can be approved, its creator may email a valid government-issued ID to Admitly solely so an administrator can verify identity and that the creator is at least 18. Verification applies to the creator account, so a verified creator generally does not need to resubmit identification for each future event. Admitly may require reverification where reasonably necessary for security, fraud prevention, legal compliance, verification error, or identity concerns. The ID image is not uploaded to or stored in the Admitly application or database and must be deleted from the verification email account as soon as verification is complete. Admitly retains only user verification status and history, verifier administrator ID, verification time, optional non-document notes, and an event approval-time verification snapshot. Admitly does not retain a date of birth or government ID number.' },
  ...
  organizers: {
    ...
    sections: [
      { heading: 'Age and identity verification', body: 'An event creator must be at least 18 and have their creator account verified before an event may be approved or published. Government ID is submitted separately by email for manual review. Once verified, a creator generally does not need to resubmit identification for each future event, although Admitly may require reverification for security, fraud prevention, legal compliance, verification error, or identity concerns. The ID image is not stored in the Admitly application or database and must be deleted from the verification email account after review; only verification metadata, history, and event approval-time evidence are retained.' },
